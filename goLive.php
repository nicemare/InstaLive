...
registerArgument($helpData, $argv, "dandyMode", "Dandy Mode", "Starts and uses a website console rather than the command line.", "w", "dandy");
...
function preparationFlow($helper, $args, $commandData, $streamTotalSec = 0, $autoPin = null)
{
    ...
    if (webMode || dandyMode && !file_exists('static/')) {
    ...
    try {
        ...
        if (!Utils::isRecovery()) {
            if(dandyMode){
                ...
                Utils::log("Video Source (File/Url path or Youtube Url) : ");
                $vidInput = Utils::promptInput();
                $isyt = preg_match('/(youtube.com|youtu.be)/', $vidInput);
                if($isyt){
                    Utils::log("Getting video from youtube...");
                    $videoFilename = getYTvid($vidInput);
                } else {
                    $videoFilename = $vidInput;
                }
        ...
        //Stream Pause
        if ((!$obsAutomation || obsNoStream || $helper->slobsPresent) && !Utils::isRecovery() && !bypassPause && !dandyMode) {
...
function livestreamingFlow($ig, $broadcastId, $streamUrl, $streamKey, $obsAuto, $helper, $streamTotalSec, $autoPin, $args, $commandData, $startCommentTs = 0, $startLikeTs = 0, $startingQuestion = -1, $startingTime = -1)
{
    if (bypassCheck && !Utils::isMac() && !Utils::isWindows()) {
        ...
        if (webMode) {
            ...
            shell_exec("php -S localhost:8000 webServer.php" . (autoArchive === true ? " -a" : "") . (autoDiscard === true ? " -d" : ""));
        }
        $cmd = "";
        if (Utils::isWindows()) {
...
function endLivestreamFlow($ig, $broadcastId, $archived, $obsAuto, $helper, $pid, $commentCount, $likeCount, $likeBurstCount, $exit = true)
{
    ...
    if ($exit) {
        Utils::log("Goodbye: Thanks for streaming with InstagramLive-PHP!\nOriginal Source by JRoy\nModified by Dandy");
        sleep(2);
        exit(0);
    }
}
...
function getYoutubeID($url) {
    $regExp = "/(?:youtube\.com\/\S*(?:(?:\/e(?:mbed))?\/|watch\?(?:\S*?&?v\=))|youtu\.be\/)([a-zA-Z0-9_-]{6,11})/";
    preg_match($regExp, $url, $matches);
    return $matches[1];
}

function getYTvid($full_url){
    $id = getYoutubeID($full_url);
    if ($id != "") {
